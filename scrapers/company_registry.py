...
_TIER_2: list[CompanyRecord] = [
    CompanyRecord(
        ...
        notes="Phase 10 Step 2: live-verified, but this tenant has no `workerSubType` facet at all (only Job_Area/timeType/Regular_Temporary/location facets), and a `searchText=\"intern\"` fallback returns 846 total results - well over the 500-result (25-page) pagination safety cap, and would silently truncate rather than complete. No clean, bounded implementation exists today; deferred rather than forced. Revisit if a narrower Workday facet appears, or if a higher request-volume budget is deliberately approved. Phase 10 Step 5 re-verification: still no employment-type facet (board grew to 1094 total); `searchText=\"intern\"` now returns 853 (still over cap, and matches unrelated results like \"One International Center\" - a substring match, not a real title filter); `searchText=\"summer analyst\"` returns only 6 (too narrow, would miss most real postings); `searchText=\"student program\"` returns 0. No safe bounded filter exists. Still deferred - same root cause, not stale research.",
        business_relevance_score=8, company_reputation_score=6, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=4, evidence_score=9,
    ),
    CompanyRecord(
        ...
        notes="Phase 10 Step 2: live-verified, but this tenant has no `workerSubType` facet (only `jobFamilyGroup`, a department breakdown with no employment-type dimension), and `searchText=\"intern\"` returns 1516 total results - far over the safe pagination cap. Same reasoning as Truist above; deferred rather than forced. Phase 10 Step 5 re-verification: board grew to 1713 total, still no employment-type facet; `searchText=\"intern\"` now returns 1489 (essentially unfiltered - the search appears to match broadly, not as a literal title substring); `searchText=\"co-op\"` returns 1713, identical to the totally unfiltered count, confirming this tenant's searchText parameter provides no real narrowing at all. Still deferred - same root cause, not stale research.",
    ...
    CompanyRecord(
        ...
        notes="Phase 10 Step 2: this tenant currently redirects to Workday's own maintenance page (`community.workday.com/maintenance-page`, HTTP 500/422 on both robots.txt and the jobs API) - confirmed this is specific to the \"verizon\" tenant, not a wd5-pod-wide outage (Abbott, already in production on the same wd5 pod, responded normally). Deferred rather than guessing an alternate tenant slug; retry in a future pass. Phase 10 Step 5 re-verification (retried after roughly a week): still HTTP 422 on the jobs API and HTTP 500 on both robots.txt and the career page itself, with a real browser User-Agent header; Abbott on the same wd5 pod re-confirmed working normally in the same session. This is a genuine persistent tenant-level outage, not a transient blip - still deferred.",
        business_relevance_score=6, company_reputation_score=8, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=3, evidence_score=7,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        notes="Phase 10 Step 2: live-verified, but this tenant's `workerSubType` facet parameter is repurposed to carry a \"Skills\" facet instead of job type (a real, tenant-specific anomaly - confirmed by inspecting the facet's own `descriptor` field). `jobFamilyGroup` is a functional-area breakdown (Software Engineering, Consulting, Finance, etc.) with no employment-type dimension, and `searchText=\"intern\"` is capped at the display maximum (2000 total) - the board is both too large and lacks any reliable narrowing mechanism. Deferred rather than forced. Phase 10 Step 5 re-verification: `workerSubType` still repurposed as \"Skills\" (55 values: Release Management, SQA, Change Management, etc. - confirmed still not an employment-type facet); `jobFamilyGroup` still a 40-value functional-area breakdown with no student/intern option; `searchText=\"intern\"` still capped at the 2000 display maximum; `searchText=\"summer analyst\"` returns 516 (still over the pagination cap); `searchText=\"student\"` returns 82 (bounded, but mixes in irrelevant part-time roles like \"Student Receptionist\" with no reliable way to separate them from real internships pre-fetch). Still deferred - same root cause, not stale research.",
        business_relevance_score=8, company_reputation_score=8, internship_volume_score=8,
        function_breadth_score=7, ats_accessibility_score=3, evidence_score=9,
    ),
    CompanyRecord(
...
_TIER_3: list[CompanyRecord] = [
    ...
    CompanyRecord(
        ...
        notes="Phase 10 Step 3: live-verified, and real intern postings clearly exist (Undergraduate Marketing/Business Analytics/Sales Intern titles found via direct search) - but no `workerSubType` facet value tags them (only Regular=619, Temporary=14), and `searchText=\"intern\"` returns all 633 total with zero narrowing (unlike every other tenant checked this phase), meaning it provides no benefit over an unfiltered scan. Total board (633) exceeds the safe pagination cap either way. No clean, bounded implementation exists today; deferred rather than forced. Phase 10 Step 5 re-verification: this tenant's facet set has since been restructured (Job_Area replaced by a new 30-value `jobFamilyGroup`, plus new Location_Country/remoteType/startDate facets) - a real infrastructure change since Step 3 - but the same core blocker persists: `workerSubType` is still only Regular=613/Temporary=12, no Intern value; `searchText=\"intern\"` and `searchText=\"co-op\"` both still return the full unfiltered 625 total with zero narrowing effect, confirming this tenant's search still doesn't filter by title at all. Still deferred - same root cause under a changed but equally blocked facet structure.",
        business_relevance_score=7, company_reputation_score=8, internship_volume_score=6,
        function_breadth_score=7, ats_accessibility_score=2, evidence_score=9,
    ),
    CompanyRecord(
...
_TIER_4: list[CompanyRecord] = [
    ...
    CompanyRecord(
        ...
        notes="Big 4 peer of PwC (Tier 2, confirmed Workday) and Accenture/Guidehouse (Tier 2, confirmed Workday). Phase 10 Step 7: live-checked - primary US site is apply.deloitte.com (distinct from Deloitte Ireland's confirmed-Workday deloitteie.wd3.myworkdayjobs.com tenant, which is a different entity and not usable for US postings); apply.deloitte.com's own URL structure does not match Workday's, and no direct myworkdayjobs.com evidence for the US entity was found. Do not assume Workday for the US entity - platform remains unconfirmed.",
    ...
    CompanyRecord(
        ...
        notes="Phase 10 Step 7: live-checked (careers.ey.com, usearlycareers.ey.com) - no myworkdayjobs.com or other identifiable ATS evidence found. Platform remains unconfirmed.",
        business_relevance_score=9, company_reputation_score=9, internship_volume_score=8,
        function_breadth_score=7, ats_accessibility_score=3, evidence_score=3,
    ),
    CompanyRecord(
        ...
        notes="Phase 10 Step 7: live-checked and found MULTIPLE distinct platforms across divisions, none clearly the main internship board: Oracle Cloud HCM for \"KPMG Global Services\" specifically (ejgk.fa.em2.oraclecloud.com - a specific division, not the general US firm), and Avature for campus recruiting (uskpmgats.avature.net/campusApply - internships are campus recruiting, so this is plausibly the relevant one, but Avature's public-access model was not evaluated this session). Genuinely ambiguous which platform serves the real US internship program - not guessed.",
...
_TIER_5: list[CompanyRecord] = [
    ...
    CompanyRecord(
        ...
        ats_config={"tenant": "bah", "site": "BAH_Jobs", "intern_facet_id": "12aea2bdfbdb10dbf53be9b8e38a2a1d"},
        notes="Phase 10 Step 7: live-verified `workerSubType` facet for \"Intern - Paid\" (24 open postings at verification time). Board skews government/defense-consulting heavy, but the company's own site confirms structured internship programs (Summer Games virtual program, in-person summer hire program). 9 relevant business-adjacent internships created on first local run, zero errors. Implemented.",
        scraper_module="scrapers.companies.boozallen",
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4). Mid-size management/technology consultancy with a known structured internship program. Phase 10 Step 7: live-checked - applications go through a third-party portal (acceptthechallenge.com) rather than myworkdayjobs.com; no ATS platform evidence found. Still unconfirmed.",
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 5: live-checked and the US entity (Grant Thornton LLP) actually runs on Oracle Cloud HCM (`ehzq.fa.us2.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX_1/jobs`), the same platform independently found at JPMorgan Chase this same phase - only the Australian entity (`grantthorntonaus.wd105.myworkdayjobs.com`) is genuinely on Workday. Do not implement against Workday for the US entity. See the JPMorgan Chase entry for the Oracle Cloud HCM finding.",
        business_relevance_score=8, company_reputation_score=6, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=1, evidence_score=6,
    ),
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4). Bulge-bracket IB with one of the largest, most structured Summer Analyst programs in finance. Phase 10 Step 7: live-checked (goldmansachs.com/careers) - no ATS platform evidence found, consistent with a fully custom/proprietary recruiting platform. Still unconfirmed.",
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4). Largest US bank by assets; massive structured Summer Analyst program across IB/Markets/Corporate. Phase 10 Step 5: live-checked - the careers site (jpmorganchase.com/careers, formerly careers.jpmorgan.com) runs on Oracle Cloud HCM (Oracle Recruiting Cloud), confirmed via `jpmc.fa.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX_1001/requisitions` - not Greenhouse/Workday/Lever. Not a Greenhouse/Workday/Lever board, so no scraper was built this phase per the explicit \"stop and report, don't improvise\" instruction. See docs/architecture.md for the full future-ATS-integration writeup (also found independently at Grant Thornton LLP's US entity this same phase).",
        business_relevance_score=10, company_reputation_score=10, internship_volume_score=10,
        function_breadth_score=8, ats_accessibility_score=1, evidence_score=6,
    ),
    CompanyRecord(
        name="Morgan Stanley", slug="morgan-stanley", industry="Investment Banking",
        ats=ATSPlatform.UNKNOWN, tier=CompanyTier.TIER_5, status=CompanyStatus.NEEDS_REVIEW,
        website_url="https://www.morganstanley.com",
        notes="Candidate pool (Step 4). Bulge-bracket IB/wealth management; large Summer Analyst program. Phase 10 Step 7: live-checked (morganstanley.com/people) - no ATS platform evidence found. Still unconfirmed.",
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4). Bulge-bracket bank with a very large campus recruiting program spanning IB, Global Markets, and retail banking. Phase 10 Step 7: live-checked - a real finding, not a guess: experienced/lateral roles ARE on Workday (ghr.wd1.myworkdayjobs.com/Lateral-US, confirmed), but campus/early-careers internships specifically route through a SEPARATE platform (bankcampuscareers.tal.net - Cornerstone OnDemand's TalentLink product). Implementing the Workday board would not actually capture internships. Do not implement against Workday for this company - tal.net's public-access model was not evaluated this session.",
        business_relevance_score=9, company_reputation_score=9, internship_volume_score=9,
    ...
    CompanyRecord(
        ...
        ats_config={"tenant": "citi", "site": "2", "search_text": "summer analyst"},
        notes="Phase 10 Step 7: Step 4's \"custom platform\" guess corrected - live-checked and this company actually IS on Workday (tenant \"citi\", site \"2\" - an unusual but genuine numeric site identifier, unlike most other bulge-bracket banks checked this project which are on Oracle Cloud HCM or fully proprietary platforms). `workerSubType` facet has a clean but thin \"Intern (Paid)\" value (7); `searchText=\"summer analyst\"` is both cleaner and far more complete (66 results) - Citi's own site brands its program \"Summer Analyst\". 40 relevant internships created on first local run, zero errors. Implemented.",
        scraper_module="scrapers.companies.citigroup",
        business_relevance_score=9, company_reputation_score=9, internship_volume_score=8,
        function_breadth_score=8, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "wf", "site": "WellsFargoJobs", "intern_facet_id": "2d264dd4beb00100f05a7d60652a0000"},
        notes="Phase 10 Step 6: live-verified `workerSubType` facet for \"Intern (Fixed Term) (Trainee)\" (19 open postings at verification time). Real postings directly confirmed via search (2027 Summer Internship - Corporate & Investment Banking COO, 2026 Finance Internship Program, 2026 Commercial Banking Summer Internship). 17 relevant internships created on first local run, zero errors. Implemented.",
        scraper_module="scrapers.companies.wellsfargo",
        business_relevance_score=8, company_reputation_score=7, internship_volume_score=7,
        function_breadth_score=7, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        name="Deutsche Bank", slug="deutsche-bank", industry="Financial Services",
        ats=ATSPlatform.WORKDAY, tier=CompanyTier.TIER_3, status=CompanyStatus.IMPLEMENTED,
        career_url="https://db.wd3.myworkdayjobs.com/DBWebsite",
        website_url="https://www.db.com",
        ats_config={"tenant": "db", "site": "DBWebsite", "intern_facet_id": "645e861bc53a01f8c083feaefb3a1c09"},
        scraper_module="scrapers.companies.deutschebank",
        notes="Phase 10 Step 8: live-verified `workerSubType` facet for \"Q (Interns / Practical Work Exp.) (Fixed Term)\" (27 open postings at verification time). Postings skew international (Milan, Sao Paulo, Luxembourg, Shanghai, Beijing). 26 relevant internships created on first local run (1 in-run dedupe collision, correctly skipped, not an error), zero other errors. Implemented.",
        business_relevance_score=9, company_reputation_score=7, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        name="The PNC Financial Services Group", slug="pnc-financial", industry="Financial Services",
        ats=ATSPlatform.WORKDAY, tier=CompanyTier.TIER_3, status=CompanyStatus.IMPLEMENTED,
        career_url="https://pnc.wd5.myworkdayjobs.com/External",
        website_url="https://www.pnc.com",
        ats_config={"tenant": "pnc", "site": "External", "intern_facet_id": "212fee894bd90147f848029e0a07d40c"},
        notes="Phase 10 Step 6: live-verified `workerSubType` facet for \"Intern (Seasonal) (Trainee)\" (24 open postings at verification time). Real posting directly confirmed via search (\"Asset Management Group Undergraduate Intern\"). 22 relevant internships created on first local run, zero errors. Implemented.",
        scraper_module="scrapers.companies.pnc",
        business_relevance_score=8, company_reputation_score=6, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
        name="U.S. Bancorp", slug="us-bancorp", industry="Financial Services",
        ats=ATSPlatform.WORKDAY, tier=CompanyTier.TIER_5, status=CompanyStatus.NEEDS_REVIEW,
        career_url="https://usbank.wd1.myworkdayjobs.com/US_Bank_Careers",
        website_url="https://www.usbank.com",
        ats_config={"tenant": "usbank", "site": "US_Bank_Careers"},
        notes="Phase 10 Step 7: live-verified real Workday tenant (confirmed, not guessed), but no safe bounded path: `workerSubType` facet has only Regular/Temporary (no Intern value), and `searchText=\"intern\"` returns 1371 of 1466 total with heavy false-positive noise (e.g. \"International Banking Ops\"). Too large and too noisy to trust. Deferred, not forced.",
        business_relevance_score=8, company_reputation_score=6, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=3, evidence_score=7,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "capitalone", "site": "Capital_One", "intern_facet_id": "a12c70bf789e10572aab83c4780919ad"},
        notes="Phase 10 Step 7: live-verified. Hosted on an unusual pod (wd12 - Capital One's own site notes a recent migration to this URL). `workerSubType` facet has a clean \"Intern\" value (12 open postings at verification time). Real postings directly confirmed via search (\"Product Development Internship Program - Summer 2027\", \"Analyst Early Internship Program - Summer 2027\"). 8 relevant internships created on first local run, zero errors. Implemented.",
        scraper_module="scrapers.companies.capitalone",
    ...
    CompanyRecord(
        ...
        ats=ATSPlatform.UNKNOWN, tier=CompanyTier.TIER_5, status=CompanyStatus.NEEDS_REVIEW,
        website_url="https://www.schwabjobs.com",
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 6: live-checked and no myworkdayjobs.com reference was found - the careers site is the custom-branded schwabjobs.com with no platform evidence surfaced this session. ATS unconfirmed; do not assume Workday for this company.",
        business_relevance_score=8, company_reputation_score=7, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=2, evidence_score=4,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        name="BlackRock, Inc.", slug="blackrock", industry="Investment Management",
        ats=ATSPlatform.WORKDAY, tier=CompanyTier.TIER_3, status=CompanyStatus.IMPLEMENTED,
        career_url="https://blackrock.wd1.myworkdayjobs.com/BlackRock_Professional",
        website_url="https://www.blackrock.com",
        ats_config={"tenant": "blackrock", "site": "BlackRock_Professional"},
        notes="Phase 10 Step 5: live-verified. No `workerSubType` value tags interns (only Fixed Term=6, Regular=309), and `searchText=\"intern\"` provides no real narrowing (200 of 315 total, dominated by unrelated titles). Board is small (315 total), scanned in full. Only 2 postings matched the client-side title filter at verification time - a live snapshot fact for a company between major recruiting cycles, not a scraper defect; implemented anyway since the endpoint and logic are correct. Implemented.",
        scraper_module="scrapers.companies.blackrock",
        business_relevance_score=9, company_reputation_score=9, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "vanguard", "site": "vanguard_external"},
        notes="Phase 10 Step 6: live-verified. No `workerSubType` Intern value (Regular=435, Contractor=13); `searchText=\"intern\"` noisy (matches \"International Tax Specialist\"). Board is 448 total, within the pagination safety cap, scanned in full. Real intern postings directly confirmed via search (\"College to Corporate Internship - Investment Systems\", \"College to Corporate IT Internship - Data Science\"). 11 relevant internships created on first local run, zero errors. Implemented.",
        scraper_module="scrapers.companies.vanguard",
    ...
    CompanyRecord(
        ...
        ats_config={"tenant": "fmr", "site": "FidelityCareers", "search_text": "intern"},
        notes="Phase 10 Step 5: live-verified. No `workerSubType` facet exists on this tenant; `searchText=\"intern\"` narrows the 532-total board to 168, within the pagination safety cap. Zero currently-open postings matched the client-side title filter at verification time - Fidelity's own site states Summer 2026 internship applications are closed and Summer 2027 opens Fall 2026, a real recruiting-calendar snapshot, not a scraper defect. Implemented anyway since the endpoint and logic are correct and will pick up new postings automatically.",
        scraper_module="scrapers.companies.fidelity",
        business_relevance_score=8, company_reputation_score=8, internship_volume_score=7,
        function_breadth_score=7, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "statestreet", "site": "Global"},
        notes="Phase 10 Step 7: live-verified real Workday tenant (confirmed via a real \"State Street Summer Internship Program (Massachusetts)\" posting found directly), but no safe bounded path: no employment-type facet exists at all, and `searchText=\"intern\"` returns 659 of 1179 total with heavy false-positive noise (e.g. \"Working Student\", \"International Capital\"). Too large and too noisy to trust. Deferred, not forced.",
        business_relevance_score=8, company_reputation_score=6, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=3, evidence_score=7,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        ats_config={"tenant": "troweprice", "site": "TRowePrice"},
        notes="Phase 10 Step 6: live-verified. No `workerSubType` facet; `searchText=\"intern\"` noisy (matches \"Internal Consultant\"). Board is small (128 total), scanned in full. Real intern postings directly confirmed via search (Equity Research Associate Analyst Internship, Global Product Internship Program, Client Services Internship Program) but zero currently open at verification time - a live snapshot fact (posting churn), not a defect; the false-positive \"Internal\" titles correctly did not match the client-side title filter. Implemented anyway since the endpoint and logic are correct.",
        scraper_module="scrapers.companies.troweprice",
    ...
    CompanyRecord(
        name="Northern Trust", slug="northern-trust", industry="Financial Services",
        ats=ATSPlatform.WORKDAY, tier=CompanyTier.TIER_3, status=CompanyStatus.IMPLEMENTED,
        career_url="https://ntrs.wd1.myworkdayjobs.com/northerntrust",
        website_url="https://www.northerntrust.com",
        ats_config={"tenant": "ntrs", "site": "northerntrust", "intern_facet_id": "333f5ad2433c1000f77c32732b4a0000"},
        scraper_module="scrapers.companies.northerntrust",
        notes="Phase 10 Step 6 found no `workerSubType` Intern value on this tenant (only \"Regular\") and deferred it. Re-verified live in Phase 10 Step 8: a genuine \"Intern (Fixed Term) (Trainee)\" facet value now exists (19 open postings) - Workday facets are driven by currently-open requisitions, so this is real posting-cycle churn (internship season opening), not a correction of a prior mistake. 18 relevant internships created on first local run (1 skipped, a technical-role exclusion working as intended), zero errors. Implemented.",
        business_relevance_score=8, company_reputation_score=6, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        name="Blackstone Inc.", slug="blackstone", industry="Financial Services",
        ats=ATSPlatform.WORKDAY, tier=CompanyTier.TIER_3, status=CompanyStatus.IMPLEMENTED,
        career_url="https://blackstone.wd1.myworkdayjobs.com/Blackstone_Campus_Careers",
        website_url="https://www.blackstone.com",
        ats_config={"tenant": "blackstone", "site": "Blackstone_Campus_Careers"},
        scraper_module="scrapers.companies.blackstone",
        notes="Phase 10 Step 8: live-verified a dedicated campus-recruiting Workday site (\"Blackstone_Campus_Careers\", robots.txt explicitly `Allow:`), separate from the general \"Blackstone_Careers\" board - small enough (28 total) to fetch whole-board with no facet, same pattern as CIBC/Piper Sandler. Covers Private Equity, Credit & Insurance, Real Estate, Private Wealth, and Finance/Internal Audit tracks. 24 relevant internships created on first local run (3 skipped - Data Engineer/Software Engineer/Cybersecurity Summer Analyst, correctly excluded as technical), zero errors. Implemented.",
        business_relevance_score=9, company_reputation_score=10, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 5: live-checked and this company actually runs on iCIMS (`careers-statefarm.icims.com`), not Workday - the Workday estimate was wrong for this specific company despite being a reasonable sector-wide prior. iCIMS is used by a meaningful share of Fortune 500 employers; not yet evaluated for a genuine public/unauthenticated job-listing endpoint - do not implement until that's confirmed.",
        business_relevance_score=7, company_reputation_score=7, internship_volume_score=7,
        function_breadth_score=7, ats_accessibility_score=2, evidence_score=6,
    ),
    CompanyRecord(
        name="Progressive Corporation", slug="progressive", industry="Insurance",
        ats=ATSPlatform.UNKNOWN, tier=CompanyTier.TIER_5, status=CompanyStatus.NEEDS_REVIEW,
        website_url="https://www.progressive.com",
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 6: live-checked and this company actually runs on Jobvite (confirmed via Jobvite's own customer case study naming Progressive as their ATS/CRM), not Workday. Jobvite is a new-to-this-project ATS platform; its public-access model was not evaluated this session - do not implement until confirmed.",
        business_relevance_score=7, company_reputation_score=7, internship_volume_score=7,
        function_breadth_score=7, ats_accessibility_score=2, evidence_score=6,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "allstate", "site": "allstate_careers"},
        notes="Phase 10 Step 7: live-verified. No `workerSubType` Intern value (Regular=464, Fixed Term Contract=1); board is 465 total, within the pagination safety cap, scanned in full. Real intern postings (e.g. Data Science Internship) confirmed via search but zero currently open at verification time - checked exhaustively across the full board, a live posting-churn snapshot, not a defect. Implemented anyway since the endpoint and logic are correct.",
        scraper_module="scrapers.companies.allstate",
        business_relevance_score=7, company_reputation_score=7, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
        name="The Travelers Companies", slug="travelers", industry="Insurance",
        ats=ATSPlatform.WORKDAY, tier=CompanyTier.TIER_3, status=CompanyStatus.IMPLEMENTED,
        career_url="https://travelers.wd5.myworkdayjobs.com/External",
        website_url="https://www.travelers.com",
        ats_config={"tenant": "travelers", "site": "External"},
        notes="Phase 10 Step 7: live-verified. `workerSubType` \"Intern Program (Fixed Term)\" facet only tags 1 posting despite real confirmed postings appearing directly in raw results (Actuarial Leadership Development Program Intern, Business Insurance Underwriting Program Internship, Summer Intern - Specialty Insurance) - board is small (354 total), scanned in full instead. 1 relevant internship created on first local run (a real, live snapshot - matches the thin facet observation). Implemented.",
        scraper_module="scrapers.companies.travelers",
        business_relevance_score=7, company_reputation_score=6, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 7: live-checked (jobs.libertymutualgroup.com, searchjobs.libertymutualgroup.com) and no myworkdayjobs.com evidence was found. Platform remains unconfirmed.",
    ...
    CompanyRecord(
        ...
        ats_config={"tenant": "usaa", "site": "USAAJOBSWD"},
        notes="Phase 10 Step 6: live-verified. No `workerSubType` Intern value (Regular=169 only); board is small, scanned in full. USAA runs a well-documented structured 10-week summer internship across two recruiting waves (Fall Aug-Nov, Spring Jan-Mar) - zero currently open at verification time is consistent with being between waves, not a defect. Implemented anyway since the endpoint and logic are correct.",
        scraper_module="scrapers.companies.usaa",
        business_relevance_score=7, company_reputation_score=7, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 6: live-checked and no Workday reference was found - the careers site is metlifecareers.com with no myworkdayjobs.com evidence in this session's research. ATS unconfirmed; do not assume Workday for this company.",
        business_relevance_score=7, company_reputation_score=7, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=2, evidence_score=4,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "pru", "site": "Careers"},
        notes="Phase 10 Step 6: live-verified. No employment-type facet exists; `searchText=\"intern\"` noisy (matches \"Integrated Disability Claims Examiner\"). Board is small (147 total), scanned in full. Real intern postings directly confirmed via search (2026 Sales Internship Program, 2026 Corporate Finance Internship Program, Global Retirement & Insurance Leadership Development Internship Program). 2 relevant internships created on first local run, zero errors. Implemented.",
        scraper_module="scrapers.companies.prudential",
        business_relevance_score=7, company_reputation_score=7, internship_volume_score=6,
        function_breadth_score=7, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "mmc", "site": "MMC", "intern_facet_id": "840296504fcf01ec4132442dab4aa60e"},
        notes="Phase 10 Step 6: live-verified `workerSubType` facet for \"Intern (Fixed Term) (Trainee)\" (47 open postings at verification time). Real postings directly confirmed via search (Accounting Intern, Analytics Solutions Intern, Mercer Marsh Benefits Consulting Summer Internship Programme, Career Consulting Summer Associate - MBA track). 23 relevant internships created on first local run, zero errors. Global tenant; relies on the existing frontend US/Canada display filter. Implemented.",
        scraper_module="scrapers.companies.marshmclennan",
        business_relevance_score=8, company_reputation_score=7, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 6: live-checked (jobs.aon.com) and this company actually runs on Jibe (part of Phenom People), confirmed via jibecdn.com asset URLs in the page - not Workday. Note: search results for \"Aon myworkdayjobs\" surfaced only the unrelated American Oncology Network (also abbreviated \"AON\") - a real name-collision trap, correctly not used. Jibe/Phenom's public-access model was not evaluated this session - do not implement until confirmed.",
        business_relevance_score=8, company_reputation_score=7, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=2, evidence_score=6,
    ),
    CompanyRecord(
        name="Willis Towers Watson", slug="willis-towers-watson", industry="Insurance",
        ats=ATSPlatform.UNKNOWN, tier=CompanyTier.TIER_5, status=CompanyStatus.NEEDS_REVIEW,
        website_url="https://www.wtwco.com",
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 7: live-checked (careers.wtwco.com) and no myworkdayjobs.com evidence was found - the company's own hiring-process page states applications go only through its own Careers site. Platform remains unconfirmed.",
        business_relevance_score=8, company_reputation_score=6, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=2, evidence_score=4,
    ),
    ...
    CompanyRecord(
        ...
        notes="Phase 10 Step 7 found a shared \"en_US/careers/JobDetail/{title}/{id}\" URL pattern with CBRE, plausibly a shared vendor. Phase 10 Step 8 confirmed the vendor directly: Avature (`ibmglobal.avature.net`, white-labeled at careers.ibm.com; confirmed via IBM's own live job URLs, e.g. \"Automation & Test Development Intern or Co-op 2026\"). robots.txt explicitly ALLOWs the /careers/* path, but the actual pages return HTTP 202 with a completely empty body to a plain, honestly-identified HTTP request (this project's own scraper User-Agent included) - a bot-management/WAF challenge, not real content, and no public JSON API could be found. No headless-browser dependency exists in this project's architecture, and adding one to get past a bot challenge would cross the project's explicit no-bypass rule. Not implementable without a real architecture change and would still require solving a bot challenge. Deferred.",
        business_relevance_score=6, company_reputation_score=7, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=1, evidence_score=9,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4). Large pharmaceutical company; Workday estimated by analogy to Abbott/Medtronic. Phase 10 Step 7: a Workday tenant (lilly.wd5.myworkdayjobs.com) was found via search with real historical posting URLs, but both plausible site names tried (\"CMP\", \"LLY\") returned HTTP 422 from the API and HTTP 500 from the career page itself directly - a genuine current outage/broken tenant, not a wrong guess. Lilly's own internship page also directs applicants to a RippleMatch talent community rather than a direct job search, suggesting the primary pipeline may have moved. Deferred rather than guessing a third site name.",
        business_relevance_score=6, company_reputation_score=8, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=3, evidence_score=6,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "jj", "site": "JJ", "intern_facet_id": ["ca32b336cc4a472a863c62c03f6e63b6", "c8cdc206cb91013d31d96bb30f801468"]},
        notes="Phase 10 Step 5: live-verified. `workerSubType` facet has two genuine student-program values used together via the existing multi-facet-id OR support - \"Intern (Fixed Term)\" (42 open postings) and \"Co-Op (Fixed Term)\" (18 open postings) at verification time. Strong business-function breadth confirmed via `jobFamilyGroup`: Finance=136, Marketing=62, Human Resources=34, Strategy & Corporate Development=14, Procurement=30. Global tenant; relies on the existing frontend US/Canada display filter. Implemented.",
        scraper_module="scrapers.companies.jnj",
        business_relevance_score=7, company_reputation_score=9, internship_volume_score=7,
        function_breadth_score=7, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "pfizer", "site": "PfizerCareers", "search_text": "intern"},
        scraper_module="scrapers.companies.pfizer",
        notes="Phase 10 Step 8: live-verified (Workday tenant confirmed, not estimated). No clean `workerSubType` Intern value and the whole board (523) is over the pagination cap, so `search_text=\"intern\"` is used (231 total, within cap). Raw search also surfaces unrelated \"Internal Medicine\" titles, correctly excluded by the existing word-boundary `INTERN_TITLE_RE` filter. 4 relevant internships created on first local run, zero errors - small yield because most global postings are non-English-titled (German \"Praktikant\"/\"Praktikum\", French \"Alternant\") and don't contain the literal word \"intern\", a known, accepted limitation shared with other global tenants. Implemented.",
    ...
    CompanyRecord(
        ...
        ats_config={"tenant": "msd", "site": "SearchJobs", "intern_facet_id": ["778f59ec4f0a0198986b520b021105b1", "778f59ec4f0a017df770c73a021144b1"]},
        notes="Phase 10 Step 7: live-verified. Tenant \"msd\" (Merck's international/pharma entity name, \"Merck Sharp & Dohme\") confirmed genuine via real returned postings, not guessed. `workerSubType` facet has two genuine student-program values used together via the existing multi-facet-id OR support - \"Intern/Co-op (Fixed Term)\" (42) and \"Intern/Co-Op\" (1) at verification time. 16 relevant internships created on first local run, zero errors. Global tenant; relies on the existing frontend US/Canada display filter. Implemented.",
        scraper_module="scrapers.companies.merck",
        business_relevance_score=6, company_reputation_score=8, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 5: live-checked and this company actually runs on Taleo (Oracle) with a TalentBrew front-end (`uhg.taleo.net/careersection/...`), not Workday. Taleo is a mature, widely-used enterprise ATS - not yet evaluated for a genuine public/unauthenticated job-listing endpoint - do not implement until that's confirmed.",
    ...
    CompanyRecord(
        ...
        ats_config={"tenant": "cigna", "site": "cignacareers", "intern_facet_id": "b7947bbbfff2018a3282f98c340c7508"},
        scraper_module="scrapers.companies.cigna",
        notes="Phase 10 Step 8: live-verified `workerSubType` facet for \"CoOp / Intern (Seasonal) (Trainee)\" (10 open postings at verification time). Strongest pure-business internship mix found this batch (Actuarial, Analytics/AI and MBA Finance Leadership Development Programs, Legal, Investment Management, Healthcare Policy). 9 relevant internships created on first local run, zero errors. Implemented.",
        business_relevance_score=7, company_reputation_score=6, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        ats_config={"tenant": "pg", "site": "1000", "intern_facet_id": "e294e7e48559015579693cb03207ff2b"},
        notes="Phase 10 Step 5: live-verified `workerSubType` facet for \"Intern/Co-Op (Fixed Term) (Trainee)\" (176 open postings at verification time, out of 744 total). Global tenant; relies on the existing frontend US/Canada display filter. 112 relevant business-role internships created on first local run (one harmless title+location dedupe collision, same pattern as GlobalFoundries/Airbus in Step 3). Implemented.",
        scraper_module="scrapers.companies.procterandgamble",
        business_relevance_score=9, company_reputation_score=9, internship_volume_score=8,
        function_breadth_score=8, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 6: live-checked and this company runs on MULTIPLE platforms across divisions - the main US careers login is iCIMS (uscareers-pepsico.icims.com), PepsiCo Beverages North America uses ADP (recruiting.adp.com), and a \"pbv\" Workday tenant exists for one subsidiary only. No single clean platform maps to PepsiCo's genuine main internship program; guessing which division's board is \"the real one\" would violate the no-guessing rule. Deferred - would need a targeted follow-up to confirm which entity's postings actually cover PepsiCo's flagship internship program.",
        business_relevance_score=9, company_reputation_score=9, internship_volume_score=7,
        function_breadth_score=8, ats_accessibility_score=2, evidence_score=6,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "coke", "site": "coca-cola-careers", "search_text": "intern"},
        notes="Phase 10 Step 6: live-verified. No `workerSubType` facet; `searchText=\"intern\"` narrows the 226-total board to 130, within the pagination safety cap, and returns genuinely relevant real titles (Coca-Cola Ignite Intern - Strategy, Finance Intern - Graduate). 5 relevant internships created on first local run, zero errors. Implemented.",
        scraper_module="scrapers.companies.cocacola",
    ...
    CompanyRecord(
        ...
        ats_config={"tenant": "unilever", "site": "Unilever_Early_Careers"},
        notes="Phase 10 Step 6: live-verified. Used the dedicated \"Unilever_Early_Careers\" site (distinct from the separate \"Unilever_Experienced_Professionals\" site also found, deliberately not used - targets experienced hires). Only 4 total postings and no `workerSubType` Intern value at verification time - a real, thin, live snapshot (this tenant's internship recruiting is clearly seasonal), scanned in full. Real internship-titled postings directly confirmed via search (Unilever Marketing Internship, 2025 Summer Formulation Internship, YES Programme 12-month Internship) even though zero matched on this run. Implemented anyway since the endpoint and logic are correct.",
        scraper_module="scrapers.companies.unilever",
        business_relevance_score=8, company_reputation_score=8, internship_volume_score=6,
        function_breadth_score=7, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 7: live-checked (jobs.colgate.com) and no myworkdayjobs.com evidence was found. Platform remains unconfirmed.",
        business_relevance_score=8, company_reputation_score=6, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=2, evidence_score=4,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "mdlz", "site": "External", "intern_facet_id": "05fb736b3afb0194b97ef64ab8003784"},
        notes="Phase 10 Step 7: live-verified `workerSubType` facet for \"Intern (Fixed Term)\" (51 open postings at verification time). Real posting directly confirmed via search (\"Internship Mondelez ... Sales - Category Management & Trade Promotion Assistant\"). Runs a branded \"Taste The Future\" US internship program (Sales, Supply Chain, Brand Management, ~90-100 interns/year). 29 relevant internships created on first local run, zero errors. Global tenant; relies on the existing frontend US/Canada display filter. Implemented.",
        scraper_module="scrapers.companies.mondelez",
        business_relevance_score=8, company_reputation_score=6, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "heinz", "site": "KraftHeinz_Careers_UR", "intern_facet_id": "177d1fb9ae621014944d7dbd842aef99"},
        notes="Phase 10 Step 7: live-verified. Used the dedicated \"KraftHeinz_Careers_UR\" (University Recruiting) site, distinct from the general \"KraftHeinz_Careers\" site also found - the latter targets experienced hires. `workerSubType` facet has a clean \"Student\" value (42 open postings at verification time). Real posting directly confirmed via search (\"2027 US Internship Program - Sales\"). 39 relevant internships created on first local run, zero errors. Implemented.",
        scraper_module="scrapers.companies.kraftheinz",
        business_relevance_score=8, company_reputation_score=6, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 7: live-checked (nestlejobs.com) and no myworkdayjobs.com evidence was found - instead the site references an AI-driven conversational application assistant (\"Olivia\", texting JOBS to a shortcode), likely Paradox.ai's \"Olivia\" chatbot product layered over an unidentified backend ATS. Platform genuinely unconfirmed - not guessed.",
        business_relevance_score=8, company_reputation_score=7, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=2, evidence_score=4,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        ats=ATSPlatform.WORKDAY, tier=CompanyTier.TIER_5, status=CompanyStatus.NEEDS_REVIEW,
        career_url="https://nike.wd1.myworkdayjobs.com/nke",
        website_url="https://www.nike.com",
        ats_config={"tenant": "nike", "site": "nke"},
        notes="Phase 10 Step 6: live-verified real Workday tenant (confirmed real intern postings found directly - Finance Graduate Intern, N7 Undergraduate Internship), but no safe bounded path: total board is 757, over the 500-result pagination safety cap; `workerSubType` only has an ambiguous \"Fixed Term Contract\"=180 value (not explicitly \"Intern\", would risk pulling in non-intern fixed-term contractors); `searchText=\"intern\"` is noisy (160 results dominated by unrelated titles). Deferred, not forced.",
        business_relevance_score=8, company_reputation_score=9, internship_volume_score=6,
        function_breadth_score=7, ats_accessibility_score=3, evidence_score=7,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 7: live-checked and no myworkdayjobs.com evidence was found this session. Platform remains unconfirmed.",
        business_relevance_score=8, company_reputation_score=7, internship_volume_score=4,
        function_breadth_score=6, ats_accessibility_score=2, evidence_score=4,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        ats_config={"tenant": "target", "site": "targetcareers", "intern_facet_id": "daccab9f1d25019dc0cd608d3157bd05"},
        notes="Phase 10 Step 5: live-verified `workerSubType` facet for \"Intern\" (104 open postings at verification time). 102 relevant business-role internships created on first local run, zero errors. Implemented.",
        scraper_module="scrapers.companies.target",
        business_relevance_score=7, company_reputation_score=8, internship_volume_score=7,
        function_breadth_score=7, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        ats=ATSPlatform.WORKDAY, tier=CompanyTier.TIER_5, status=CompanyStatus.NEEDS_REVIEW,
        career_url="https://homedepot.wd5.myworkdayjobs.com/CareerDepot",
        website_url="https://corporate.homedepot.com",
        ats_config={"tenant": "homedepot", "site": "CareerDepot"},
        notes="Phase 10 Step 6: live-verified real Workday tenant/site (confirmed via the company's own \"Shop Internships\" link, which proxies to this same board) - but this single \"CareerDepot\" board mixes retail/hourly store associates with corporate/salaried roles with no clean separation: `workerSubType` facet is entirely \"Regular Associate\"=1028 (the whole board, zero narrowing value), no student/intern category exists, and `searchText=\"intern\"` is noisy (303 results dominated by \"International Corporate\" substring matches) on a board too large (1028) for a safe unfiltered scan anyway. Deferred, not forced.",
        business_relevance_score=7, company_reputation_score=7, internship_volume_score=6,
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 6: live-checked (apply.ford.com/en/sites/CX_1/jobs) and this company actually runs on Oracle Cloud HCM - confirmed via \"oraclecloud\"/\"hcmUI\"/\"CandidateExperience\" markers in the page - not Workday (the earlier \"fordfoundation.wd1.myworkdayjobs.com\" reference found during research is the separate Ford Foundation philanthropy, not Ford Motor Company - a real name-collision trap, correctly not used). This is the THIRD company found on Oracle Cloud HCM this project (with JPMorgan Chase and Grant Thornton LLP's US entity) - see docs/architecture.md for the full Oracle Cloud HCM findings, including a genuinely public/unauthenticated REST API discovered this phase.",
        business_relevance_score=6, company_reputation_score=8, internship_volume_score=6,
        function_breadth_score=7, ats_accessibility_score=1, evidence_score=7,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "generalmotors", "site": "Careers_GM", "intern_facet_id": ["81219c91208501e94c213185a61a320a", "81219c91208501ec00823185a61a360a"]},
        notes="Phase 10 Step 7: live-verified `workerSubType` facet with two genuine student-program values used together via the existing multi-facet-id OR support - \"Intern (Fixed Term) (Trainee)\" (14 open postings) and \"Co-Op (Fixed Term)\" (5 open postings) at verification time. 9 relevant internships created on first local run, zero errors. Implemented.",
        scraper_module="scrapers.companies.generalmotors",
        business_relevance_score=6, company_reputation_score=8, internship_volume_score=6,
        function_breadth_score=7, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        ats_config={"tenant": "cat", "site": "CaterpillarCareers", "intern_facet_id": "3ff971b74e64011c93bf033b6e160342"},
        notes="Phase 10 Step 5: live-verified `workerSubType` facet for \"Intern - Temporary\" (13 open postings at verification time). A separate \"Student Worker/Grant Worker\" facet value (24 postings) was deliberately excluded - not confirmed to be a real internship-equivalent classification. 10 relevant business/technical-mix internships created on first local run, zero errors. Implemented.",
        scraper_module="scrapers.companies.caterpillar",
        business_relevance_score=6, company_reputation_score=7, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 5: live-checked and this company actually runs on SAP SuccessFactors (`career4.successfactors.com/careers?company=exxonmobilP`), not Workday - the sector-wide Workday prior (true for Chevron/Marathon Petroleum) did not hold here. Not yet evaluated for a genuine public/unauthenticated job-listing endpoint - do not implement until that's confirmed.",
        business_relevance_score=6, company_reputation_score=8, internship_volume_score=6,
        function_breadth_score=7, ats_accessibility_score=2, evidence_score=6,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        ats_config={"tenant": "hcmportal", "site": "Search", "intern_facet_id": "30250de825de0108126de4fb848e0000"},
        notes="Phase 10 Step 5: live-verified. Tenant name \"hcmportal\" is unusual for UPS but confirmed genuine (not guessed) by real UPS-branded postings/locations in the response (e.g. \"Accounting Intern\" at \"UPS HOUSE\", \"GLOBAL BUSINESS SERVICES-GBS\" location codes). `workerSubType` facet has a clean \"Intern\" value (17 open postings at verification time). Global tenant; relies on the existing frontend US/Canada display filter. 10 relevant business-role internships created on first local run (one harmless dedupe collision). Implemented.",
        scraper_module="scrapers.companies.ups",
        business_relevance_score=6, company_reputation_score=7, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        notes="Attempted live verification across three sessions now (Phase 10 Steps 6, 7, 8) - careers.cbre.com/en_US/careers/... consistently returns an empty HTTP 202 to a plain fetch, including robots.txt itself. Phase 10 Step 8 confirmed the vendor directly via Avature's own published case studies (\"How CBRE Empower Recruiters with Automation & Analytics\", avature.net) - internal Avature instances exist at cbreglobal.avature.net and trainingcbreglobal.avature.net, and the public careers.cbre.com is very likely a white-labeled Avature instance too (same URL pattern independently confirmed at IBM's own Avature-powered site). The empty-202 response is a bot-management/WAF challenge, not a real page - this project's architecture has no headless-browser capability and getting past it would cross the explicit no-bypass rule. No further time spent forcing this across three sessions of consistent failure.",
        business_relevance_score=8, company_reputation_score=6, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=1, evidence_score=9,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "jll", "site": "jllcareers", "intern_facet_id": "83c7fa911b4101a0f63c22b2db507809"},
        notes="Phase 10 Step 5: live-verified `workerSubType` facet for \"Intern (Fixed Term) (Trainee)\" (70 open postings at verification time). Real postings directly confirmed via search (Capital Markets Summer Internship, Accountant Intern) - strong commercial-real-estate business fit. Global tenant; relies on the existing frontend US/Canada display filter. 57 relevant internships created on first local run (one harmless dedupe collision). Implemented.",
        scraper_module="scrapers.companies.jll",
        business_relevance_score=8, company_reputation_score=6, internship_volume_score=4,
        function_breadth_score=6, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
        ...
        ats=ATSPlatform.WORKDAY, tier=CompanyTier.TIER_5, status=CompanyStatus.NEEDS_REVIEW,
        career_url="https://cw.wd1.myworkdayjobs.com/External",
        website_url="https://www.cushmanwakefield.com",
        ats_config={"tenant": "cw", "site": "External"},
        notes="Phase 10 Step 7: live-verified real Workday tenant (confirmed via real Philadelphia/NY/SF Summer Internship postings found directly), but no safe bounded path: no employment-type facet exists, total board is display-capped at 2000 (a 52,000-employee/400-office/60-country firm), and `searchText=\"intern\"` returns 737 - over the pagination cap and still noisy (matches address strings like \"International Dr\"). Re-checked live in Phase 10 Step 8: still no `workerSubType` facet at all (only `timeType`: Full time 2246 / Part time 73), whole board now 713 - same conclusion holds. Deferred, not forced.",
        business_relevance_score=8, company_reputation_score=5, internship_volume_score=4,
        function_breadth_score=6, ats_accessibility_score=2, evidence_score=7,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "prologis", "site": "Prologis_External_Careers"},
        notes="Phase 10 Step 7: live-verified. No employment-type facet exists; board is small (67 total), scanned in full (all 67 titles inspected manually - genuinely zero \"intern\" matches at verification time, confirmed across the entire board). Real intern postings directly confirmed via search (\"Intern, Real Estate Operations\", \"Prologis Summer Internship Program\", \"Intern, Energy\") but had closed by verification time - consistent with the company's own stated timeline (Summer Internship applications open in October). Implemented anyway since the endpoint and logic are correct.",
        scraper_module="scrapers.companies.prologis",
        business_relevance_score=7, company_reputation_score=5, internship_volume_score=3,
        function_breadth_score=5, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
        ...
        ats_config={"tenant": "simon", "site": "Simon"},
        notes="Phase 10 Step 7: live-verified. No `workerSubType` Intern value (Regular=165, Temporary=7); board is small (172 total), scanned in full. Real structured internship program confirmed via the company's own site (Leasing, Finance, Accounting, Marketing, IT, Legal, Design, HR tracks). 4 relevant internships created on first local run, zero errors. Implemented.",
        scraper_module="scrapers.companies.simonpropertygroup",
        business_relevance_score=7, company_reputation_score=5, internship_volume_score=3,
        function_breadth_score=5, ats_accessibility_score=9, evidence_score=10,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 6: live-checked and this company is NOT on Workday - the sector-wide prior (true for Boeing/GE Aerospace/Sierra Nevada/Airbus) did not hold here. Currently uses a mix of lockheedmartinjobs.com and an Eightfold AI-based site (lockheedmartin.eightfold.ai/careers), and is actively mid-transition to a new careers platform as of this session (their own site states candidates who applied before Aug 17 may need to reapply after launch). Deliberately not pursued further given this platform instability - revisit once the migration is complete and the new platform is confirmed.",
        business_relevance_score=5, company_reputation_score=8, internship_volume_score=6,
        function_breadth_score=5, ats_accessibility_score=1, evidence_score=6,
    ),
    CompanyRecord(
