...
_TIER_5: list[CompanyRecord] = [
    ...
    CompanyRecord(
        ...
        ats_config={"tenant": "ntrs", "site": "northerntrust"},
        notes="Phase 10 Step 6: live-verified real Workday tenant (confirmed, not guessed), but no safe bounded path exists: `workerSubType` facet has only \"Regular\"=638 (no Intern value), `jobFamilyGroup` is a pure department breakdown with no student/intern category, and `searchText=\"intern\"` returns 366 of 638 with false-positive noise (e.g. \"Inter-Capability\" substring matches) - too large and too noisy to trust. Deferred, not forced.",
        business_relevance_score=8, company_reputation_score=6, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=3, evidence_score=7,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4) Workday estimate corrected in Phase 10 Step 7: live-checked and no myworkdayjobs.com evidence was found. Instead, IBM's careers site uses a \"careers.ibm.com/en_US/careers/JobDetail/{title}/{id}\" URL pattern - the same pattern independently found at CBRE this same phase, suggesting a shared platform vendor (plausibly SAP SuccessFactors Career Site Builder). Not confirmed, not guessed.",
        business_relevance_score=6, company_reputation_score=7, internship_volume_score=6,
        function_breadth_score=6, ats_accessibility_score=2, evidence_score=4,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        notes="Candidate pool (Step 4). Largest commercial real-estate services firm. Attempted live verification twice now (Phase 10 Step 6 and Step 7) - the careers site (careers.cbre.com/en_US/careers/...) returns an empty HTTP 202 response to both a plain fetch and a direct curl request, consistent with a bot-challenge/JS-rendering gate rather than a real page. The URL pattern (\"en_US/careers/JobDetail/...\") also matches the one seen at IBM, suggesting a shared platform vendor (plausibly SAP SuccessFactors Career Site Builder, given the confirmed SuccessFactors finding at ExxonMobil used a different but related module) - not confirmed, not guessed. No further time spent forcing this given repeated failure across two sessions.",
        business_relevance_score=8, company_reputation_score=6, internship_volume_score=5,
        function_breadth_score=6, ats_accessibility_score=1, evidence_score=5,
    ),
    CompanyRecord(
    ...
    CompanyRecord(
        ...
        notes="Phase 10 Step 7: live-verified real Workday tenant (confirmed via real Philadelphia/NY/SF Summer Internship postings found directly), but no safe bounded path: no employment-type facet exists, total board is display-capped at 2000 (a 52,000-employee/400-office/60-country firm), and `searchText=\"intern\"` returns 737 - over the pagination cap and still noisy (matches address strings like \"International Dr\"). Deferred, not forced.",
        business_relevance_score=8, company_reputation_score=5, internship_volume_score=4,
